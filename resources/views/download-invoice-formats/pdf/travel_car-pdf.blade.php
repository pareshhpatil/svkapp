<html lang="en">

    <body>


        <style>
            html { 
                margin: 0px;
                margin-top: 10px;
                margin-bottom: 10px;
            }
            body
            {
                font-family: Open Sans, sans-serif;
                margin: 0px;
            }
            .form-control-plaintext{
                color: #5b4d4b;
                font-size: 13px;
                line-height: 14px;
                height: auto;
                padding: 0;
            }
            .tx-c{
                text-align: center;
            }
            .tx-r{
                text-align: right;
            }
            .bgset{
                background-color: {{$bg_color}};
                color: {{$text_color}};
            }
            .bgset2{
                background-color: {{$bg_color2}};
                color: {{$text_color2}};
            }
        </style>

        <section class="jumbotron jumbotron-features bg-transparent py-4" id="header">
            <div class="container">
                <div class="row align-items-center">


                    <div class="d-none d-lg-block col-12 col-md-12 col-lg-12 col-xl-12">
                        <table style="margin:0 auto; font-family: Open Sans, sans-serif;width: 750px; border: 1px solid grey;" align="center"  border="0" cellspacing="0" cellpadding="10">
                            <tbody><tr>
                                    <td style="font-size:15px; line-height:30px;"><table width="100%" border="0" cellspacing="0" cellpadding="5">
                                            <tbody><tr>
                                                    <td width="150"  align="center" valign="top">
                                                        @if($logo!='') <img style="max-height: 200px;max-width: 140px;" src="data:image/png;base64,{{$logo}}"> @endif
                                                    </td>
                                                    <td width="280" style="">
                                                        <span style="font-size:25px; color:#6e605d;">{{$company_name}}</span>
                                                        <span style="font-size:13px; display:block; margin-top:5px; margin-bottom:1px;line-height: 16px;">{{$merchant_address}} </span>
                                                        @if($merchant_mobile!='')<span style="font-size:13px; display:block; margin-top:1px; margin-bottom:1px;line-height: 16px;"> <div class="input-group">Contact: {{$merchant_mobile}}</div></span>@endif
                                                        @if($merchant_email!='')<span style="font-size:13px; display:block; margin-top:1px; margin-bottom:1px;line-height: 16px;"> <div class="input-group">E-mail: {{$merchant_email}}</div></span>@endif
                                                        @if($gstnumber!='')<span style="font-size:13px; display:block; margin-top:1px; margin-bottom:1px;line-height: 16px;"> <div class="input-group">GST number: {{$gstnumber}}</div></span>@endif
                                                    </td>
                                                    <td width="90" align="center" valign="top">
                                                    </td>
                                                </tr>
                                            </tbody></table>  </td>
                                </tr>
                                <tr style="border-spacing: 0px;">
                                    <td class="bgset" style="text-align: center;border-top: 1px solid grey;border-bottom: 1px solid grey;font-size:18px;">
                                        <b>TAX INVOICE</b>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="font-size:11px;  border-bottom:1px #cbcbcb;">
                                        <table border="0" cellspacing="0" cellpadding="5" style="font-size: 13px;line-height: 15px;width: 100%;">
                                            <tbody>
                                                <tr>
                                                    <td style="font-size:11px;  border-bottom:1px #cbcbcb;border-right: 1px solid #cbcbcb;width: 50%; ">
                                                        <table  border="0" cellspacing="0" cellpadding="5" style="color:#5b4d4b;font-size: 13px;line-height: 14px;">
                                                            <tbody>

                                                                @if($customer_code!='')<tr><td ><b>Customer code</b></td><td >{{$customer_code}}</td></tr>@endif
                                                                @if($name!='')<tr><td ><b>Customer name</b></td><td >{{$name}}</td></tr>@endif
                                                                @if($email!='') <tr><td ><b>Email ID</b></td><td >{{$email}}</td></tr>@endif
                                                                @if($mobile!='')<tr><td ><b>Mobile no</b></td><td >{{$mobile}}</td></tr>@endif
                                                            </tbody>
                                                        </table>
                                                    </td>
                                                    <td style="font-size:11px;  border-bottom:1px #cbcbcb;width: 50%; ">
                                                        <table  border="0" cellspacing="0" cellpadding="5" style="color:#5b4d4b;font-size: 13px;line-height: 14px;">

                                                            <tbody>
                                                                @if($bill_date!='')<tr><td ><b>Bill Date</b></td><td >{{$bill_date}}</td></tr>@endif
                                                                @if($due_date!='') <tr><td ><b>Due Date</b></td><td >{{$due_date}}</td></tr>@endif
                                                                @if($invoice_number!='') <tr><td ><b>Invoice No.</b></td><td >{{$invoice_number}}</td></tr>@endif
                                                                @if($customer_gst!='') <tr><td ><b>Customer GST No.</b></td><td >{{$customer_gst}}</td></tr>@endif
                                                            </tbody>
                                                        </table>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>

                                    </td>
                                </tr>
                                <tr>
                                    <td style="font-size:11px;  border-bottom:1px #cbcbcb;">
                                        <p style="font-size: 14px;margin: 5px; margin-bottom: 5px;">Booking details</p>
                                        <table border="0" cellspacing="0" cellpadding="5" style="font-size: 13px;line-height: 15px;width: 100%;">

                                            <tbody>
                                                <tr>
                                                    <td style="font-size:11px;  border-bottom:1px #cbcbcb;border-right: 1px solid #cbcbcb;width: 50%; ">
                                                        <table  border="0" cellspacing="0" cellpadding="5" style="color:#5b4d4b;font-size: 13px;line-height: 14px;">

                                                            <tbody>

                                                                @if($v_type!='')<tr><td ><b>Vehicle type	</b></td><td >{{$v_type}}</td></tr>@endif
                                                                @if($v_no!='')<tr><td ><b>Vehicle no. </b></td><td >{{$v_no}}</td></tr>@endif
                                                                @if($total_days!='') <tr><td ><b>Total days </b></td><td >{{$total_days}}</td></tr>@endif
                                                                @if($booked_by!='')<tr><td ><b>Booked by </b></td><td >{{$booked_by}}</td></tr>@endif
                                                                @if($duty_slip!='')<tr><td ><b>Duty slip no </b></td><td >{{$duty_slip}}</td></tr>@endif
                                                                @if($destination!='')<tr><td ><b>Destination	</b></td><td >{{$destination}}</td></tr>@endif
                                                                @if($driver_name!='')<tr><td ><b>Driver name </b></td><td >{{$driver_name}}</td></tr>@endif
                                                            </tbody>
                                                        </table>
                                                    </td>
                                                    <td style="font-size:11px;  border-bottom:1px #cbcbcb;width: 50%; ">
                                                        <table  border="0" cellspacing="0" cellpadding="5" style="color:#5b4d4b;font-size: 13px;line-height: 14px;">

                                                            <tbody>
                                                                @if($start_km!='')<tr><td ><b>Start KM </b></td><td >{{$start_km}}</td></tr>@endif
                                                                @if($end_km!='') <tr><td ><b>Close KM </b></td><td >{{$end_km}}</td></tr>@endif
                                                                @if($start_date!='') <tr><td ><b>Start date </b></td><td >{{$start_date}}</td></tr>@endif
                                                                @if($end_date!='') <tr><td ><b>End date </b></td><td >{{$end_date}}</td></tr>@endif
                                                                @if($start_time!='') <tr><td ><b>Start time </b></td><td >{{$start_time}}</td></tr>@endif
                                                                @if($end_time!='') <tr><td ><b>End time </b></td><td >{{$end_time}}</td></tr>@endif
                                                            </tbody>
                                                        </table>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>

                                    </td>
                                </tr>

                                <tr>
                                    <td style="padding-bottom: 0;padding-top: 0;">

                                        <table width="100%" border="0" cellspacing="0" cellpadding="5" style="border: 1px solid #cbcbcb;color:#5b4d4b;font-size: 13px;line-height: 15px;text-align: center;">
                                            <thead>
                                                <tr>

                                                    <th @if($time_period_label=='') colspan="2" @endif style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;min-width: 70px;text-align:left">
                                                         Particular name
                                                </th>
                                                @if($time_period_label!='')
                                                <th style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;text-align: center; ">
                                                    {{$time_period_label}}
                                                </th>
                                                @endif
                                                <th   style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;min-width: 70px;text-align: center;">
                                                    No of units
                                                </th>
                                                <th style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;text-align: center;">
                                                    Unit Price
                                                </th>

                                                <th  style="border-bottom: 1px solid #cbcbcb; text-align: right;">
                                                    {{$total_label}}
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr> 
                                                <td @if($time_period_label=='') colspan="2" @endif  style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;text-align: left;">
                                                     {{$p_description}}
                                            </td>

                                            @if($time_period_label!='')
                                            <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                                {{$p_time_period}}
                                            </td>
                                            @endif

                                            <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                                {{$p_qty}}
                                            </td>
                                            <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                                {{$p_rate}}
                                            </td>

                                            <td   style="border-bottom: 1px solid #cbcbcb;text-align: right;">
                                                @if($p_cost>0){{ number_format($p_cost, 2) }}@else 0.00 @endif
                                            </td>
                                        </tr>
                                        @if($p_cost2>0)
                                        <tr> 
                                            <td @if($time_period_label=='') colspan="2" @endif  style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;text-align: left;">
                                                 {{$p_description2}}
                                        </td>

                                        @if($time_period_label!='')
                                        <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                            {{$p_time_period2}}
                                        </td>
                                        @endif

                                        <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                            {{$p_qty2}}
                                        </td>
                                        <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                            {{$p_rate2}}
                                        </td>

                                        <td   style="border-bottom: 1px solid #cbcbcb;text-align: right;">
                                            @if($p_cost2>0){{ number_format($p_cost2, 2) }}@else 0.00 @endif
                                        </td>
                                    </tr>
                                    @endif
                                    @if($p_cost=='') @php $p_cost=0; @endphp @endif
                                    @if($p_cost2=='') @php $p_cost2=0; @endphp @endif
                                    <tr >
                                        <td   style="border-bottom: 1px solid #cbcbcb;text-align: right;"class="add-col border-0">
                                        </td>
                                        <td   style="border-bottom: 1px solid #cbcbcb;text-align: right;border-right: 1px solid #cbcbcb;"class="border-left-0" colspan="3">
                                            <b class="pull-right">Total Rs.</b>

                                        </td>


                                        <td    style="border-bottom: 1px solid #cbcbcb;text-align: right;border-right: 1px solid #cbcbcb;"class="tdpr">
                                            <b id="tp_cost">{{ number_format($p_cost + $p_cost2, 2) }}</b>

                                        </td>

                                    </tr>




                                </tbody>
                            </table>
                        </td>
                    </tr>

                    @if($tax1>0 || $tax2>0)
                    <tr>
                        <td style="padding-bottom: 0;padding-top: 0;">
                            <p style="font-size: 14px;margin: 5px; margin-bottom: 5px;">Tax details</p>
                            <table width="100%" border="0" cellspacing="0" cellpadding="5" style="border: 1px solid #cbcbcb;color:#5b4d4b;font-size: 13px;line-height: 15px;text-align: center;">
                                <thead>
                                    <tr>

                                        <th style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;min-width: 70px;">
                                            Tax name
                                        </th>
                                        <th  style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb; min-width: 70px;">
                                            Percentage
                                        </th>

                                        <th   style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;min-width: 70px;">
                                            Applicable
                                        </th>
                                        <th style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;text-align: right;">
                                            Amount
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if($tax1>0)
                                    <tr> 
                                        <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;text-align: center;">
                                            {{$tax_name1}}
                                        </td>
                                        <td  style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb; widtd: 35%;">
                                            {{$tx_percentage1}}
                                        </td>

                                        <td   style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                            {{$tx_applicable1}}
                                        </td>
                                        <td   style="border-bottom: 1px solid #cbcbcb;text-align: right;">
                                            @if($tax1>0){{ number_format($tax1, 2) }}@else 0.00 @endif
                                        </td>
                                    </tr>
                                    @endif
                                    @if($tax2>0)
                                    <tr> 
                                        <td style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;text-align: center;">
                                            {{$tax_name2}}
                                        </td>
                                        <td  style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb; widtd: 35%;">
                                            {{$tx_percentage2}}
                                        </td>

                                        <td   style="border-bottom: 1px solid #cbcbcb;border-right: 1px solid #cbcbcb;">
                                            {{$tx_applicable2}}
                                        </td>
                                        <td   style="border-bottom: 1px solid #cbcbcb;text-align: right;">
                                            @if($tax2>0){{ number_format($tax2, 2) }}@else 0.00 @endif
                                        </td>
                                    </tr>
                                    @endif
                                    <tr >

                                        <td  style="border-bottom: 1px solid #cbcbcb;text-align: right;border-right: 1px solid #cbcbcb;"class="border-left-0" colspan="3">
                                            <b class="pull-right">Tax total</b>

                                        </td>


                                        <td    style="border-bottom: 1px solid #cbcbcb;text-align: right;border-right: 1px solid #cbcbcb;"class="tdpr">
                                            <b id="tp_cost">{{ number_format($tax1 + $tax2, 2) }}</b>

                                        </td>

                                    </tr>




                                </tbody>
                            </table>
                        </td>
                    </tr>
                    @endif

                    <tr>
                        <td style="padding-top: 0;padding-bottom: 0;">
                            <table style="margin:0 auto; font-family:Verdana, Arial;" align="center" width="100%" border="0" cellspacing="0" cellpadding="10">

                                <tbody>

                                    <tr>
                                        <td colspan="5" style="font-size: 12px;"></td>
                                        <td class="bgset2"  style="font-family: Open Sans, sans-serif;width: 260px; text-align:right; font-size:16px;vertical-align: middle;">
                                            <b  style="width: 170px;">Grand Total : Rs. @if($total_due>0){{ number_format($total_due, 2) }}@else 0.00 @endif </b>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="5">@if($tnc!='')
                                            <p style="text-align: left; font-size: 12px;font-family: Open Sans, sans-serif;">
                                                {!! $tnc !!}
                                            </p>
                                            @endif</td>
                                        <td   style="width: 260px; text-align:right;line-height: 30px; font-size:14px;vertical-align: middle;">
                                        </td>
                                    </tr>


                                </tbody></table>
                        </td>
                    </tr>

                    <tr>
                        <td style="padding-bottom: 20px;padding-right: 20px;padding-left: 15px;">
                            <div class="row">
                                <div class="col-md-6 no-margin no-padding" style="text-align: left;margin-bottom: 0;font-size: 12px;float: left;width: 60%">
                                    <p style="font-family: Open Sans, sans-serif;color: #5b4d4b;"> * Note: This  is a system generated invoice. No signature required.</p>
                                </div>
                                <div class="col-md-6 no-margin no-padding" style="text-align: left;margin-bottom: 0;float: right;width: 40%;">
                                    <p style="float: right;margin-bottom: 0;margin-top: 0;font-family: Open Sans, sans-serif;">
                                        <span style="color: #5B5B5B;vertical-align: middle;margin-right: 10px;font-family: Open Sans, sans-serif !important;" >Powered by</span> 
                                        <img  src="data:image/png;base64," style="width: 130px;vertical-align: middle;" class="img-responsive pull-right powerbyimg" alt=""></p>
                                </div>
                            </div>
                            <br>
                        </td>
                    </tr>

                </tbody></table>

        </div>


    </div>
</div>
</section>







</body>
<script>
print();
</script>
</html>