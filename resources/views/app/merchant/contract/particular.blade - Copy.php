...
<div x-data="handler()">
    ...
    <div class="table-scrollable">
        <table class="table table-bordered table-hover" id="particular_table">
            ...
            @php $readonly_array=array('retainage_amount','bill_code_detail','group','bill_code');
                ...
                <template x-for="(field, index) in fields" :key="index">
                    <tr>
                        ...
                        <td style="max-width: 100px;vertical-align: middle;" @if($readonly==false) x-on:click="field.txt{{$k}} = true; @if($k=='bill_code') setAdvanceDropdownContract(index); @endif @if($k=='group') setAdvanceDropdownGroup(index+2); @endif" x-on:blur="field.txt{{$k}} = false" @else style="background-color: #f5f5f5;" @endif class="td-c onhover-border @if($k=='bill_code') col-id-no @endif" @if($k=='description' || $k=='bill_code' ) style="min-width: 100px;" @endif>
                            ...
                            <select required style="width: 100%; min-width: 200px;" onchange="billCode2()" :id="`billcode${index+2}`" x-model="field.{{$k}}" value="fee" name="{{$k}}[]" data-placeholder="Select Bill Code" class="form-control input-sm select2me productselect">
                            ...
                            @else
                            <span x-show="! field.txt{{$k}}" x-text="field.{{$k}}" :id="`b{{$k}}${index+2}`"> </span>
                            @endif
                            ...
                            @endif
                            <span x-show="field.txt{{$k}}" :id="`a{{$k}}${index+2}`">

                                @if($k=='bill_type')
                                <select required style="width: 100%; min-width: 200px;" x-on:change="setBillType(field)" :id="`bill_type${index+2}`" x-model="field.{{$k}}" value="fee" name="{{$k}}[]" data-placeholder="Select Bill Code" class="form-control input-sm select2me productselect">
                                    <option value="">Select Code</option>
                                    <template x-for="(bt, billcodeindexa) in bill_types" :key="billcodeindexa">
                                        <option x-value="bt" x-text="bt"></option>
                                    </template>
                                </select>
                                @else
        ...
        return {
            ...
            async addNewField() {
                ...
                if (_project_id > 0) {
                    ...
                    setAdvanceDropdownGroup(numrow);
                    ...
                    $('.productselect').select2({
                        ...
                        iid = $(this).attr("id");
                        this.fields[0].txtbill_type=false;
                    });
