...
@endif
    ...
    <div class="p-4  lg:p-2 md:p-2 mt-0">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-3 gap-2">
            <div class="mt-2 lg:mt-12 md:mt-12  lg:p-2 md:p-2">
   ...
   @endphp
               <div class="col-span-2">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 md:grid-cols-2 gap-2 justify-left">
                    ...
                    <div >
                        <h1 class="text-3xl  font-medium tracking-widest text-[{{$colors}}]">INVOICE</h1>
                    
                        @foreach ($metadata['invoice'] as $item)
                        @if($item['function_id']=='9')
                       
                        <p class="text-sm font-regular  text-black">{{$item['value']}}</p>
                        @endif
                        @endforeach
                        <br/>
                        @foreach ($metadata['invoice'] as $item)
            ...
            @elseif($item['datatype']=='date')
              <p class="mt-1 border-b border-gray-400"><span class="mt-1 text-black text-sm font-semibold " >{{$item['column_name']}}: </span><span class="text-sm text-black">{{date("d-M-Y", strtotime($item['value']))}}</span></p>
         @else
...
<div class="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-3 gap-2 mt-4">
    <div class="border-t-2 border-dotted border-gray-400 pt-2">
...
@if(substr($item['value'],0,7)=='http://' || substr($item['value'],0,8)=='https://')
<p class="mt-1 "><span class="mt-1 text-black text-sm font-semibold " >{{$item['column_name']}}: </span><span class="text-sm text-black"> <a target="_BlANK" class="text-sm  text-white font-semibold text-left lg:text-right md:text-right" href="{{$item['value']}}">{{$item['column_name']}}</a></span></p>

@else
@if ($item['datatype']=='money')
<p class="text-sm uppercase text-white font-semibold text-left lg:text-right md:text-right">{{number_format($item['value'],2)}} </p>
@else
<p class="mt-1 "><span class="mt-1 text-black text-sm font-semibold " >{{$item['column_name']}}: </span><span class="text-sm text-black">{{$item['value']}} @if($item['datatype']=='percent') %@endif</span></p>
...
@endif
  ...
  @endforeach
        ...
        <div class="ml-0 lg:ml-4 md:ml-4 border-t-2 border-dotted border-gray-400">
            
            @isset($info['payment_mode'])
               <p class="text-left"><span class="mt-1 text-black text-sm font-semibold" >Payment Method </span> <span class="ml-2 text-sm text-black">{{$info['payment_mode']}}</p></p>
                @endisset
                @isset($info['transaction_id'])
                <p class="text-left"><span class="mt-1 text-black text-sm font-semibold" >Transaction Number</span> <span class="ml-2 text-sm text-black">{{$info['transaction_id']}}</p></p>
                    @endisset
    </div>
...
<div class="col-span-2 border-t-2 border-dotted border-gray-400 pt-2"></div>
                ...
                <div class='overflow-x-auto w-full mt-4'>
                    ...
                    $total=0;
                        $taxtotal=0;
                @endphp
                    <table class=' mx-auto  w-full whitespace-nowrap rounded-lg bg-white divide-y-2 divide-[{{$colors}}] overflow-hidden'>
                        <thead >
            ...
            <div class=" grid grid-cols-1 lg:grid-cols-2  md:grid-cols-2   gap-0 lg:gap-2 md:gap-2 mb-2 ">
                ...
                <div >
                    <div class="flex flex-col mt-2 ">
                        ...
                        <p class="mt-0">Thank you for your purchase.</p>
