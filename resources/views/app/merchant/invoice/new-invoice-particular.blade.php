                        ...
                        <tr>
                            @foreach($particular_column as $k=>$v)
                                ...
                                @if($k!='description')
                                    ...
                                    @endif
                                    <td style="vertical-align: middle; @if($disable==true) background-color:#f5f5f5; @endif" :id="`cell_{{$k}}_${field.pint}`" @if($readonly==false)  x-on:click="field.txt{{$k}} = true; " x-on:blur="field.txt{{$k}} = false" @endif class="td-c onhover-border @if($k=='bill_code') col-id-no @endif">
                                        @if($k=='bill_code')
                                        ...
                                        <div :id="`{{$k}}${field.pint}`" x-model="field.{{$k}}" ></div>
                                        @elseif($k=='bill_type')
                                            <select required style="width: 100%; min-width: 15  0px;font-size: 12px;" :id="`bill_type${field.pint}`" x-model="field.{{$k}}" name="{{$k}}[]" data-placeholder="Select.." class="form-control select2me billTypeSelect input-sm">
                                        ...
                                        @if($k=='original_contract_amount')
                                            ...
                                            <template x-if="field.bill_type!='Calculated'">
                                        <span x-show="field.txt{{$k}}">
                                            <input :id="`{{$k}}${field.pint}`" @if($readonly==true) type="hidden" @else type="text" x-on:blur="field.txt{{$k}} = false;calc(field);saveParticulars();" @endif @keyup="removeValidationError(`{{$k}}`, `${field.pint}`)" x-model="field.{{$k}}" value="" name="{{$k}}[]" style="width: 100%;" class="form-control input-sm ">
                                        </span>
                                            </template>

                                            <input :id="`id${field.pint}`" type="hidden"  x-model="field.id" name="id[]">
                                            <input :id="`introw${field.pint}`" type="hidden" :value="index" x-model="field.pint" name="pint[]">
                            ...
                            <td class="td-c " style="vertical-align: middle;width: 60px;">
                                <button type="button" class="btn btn-xs red" @click="removeField(field, index)">&times;</button>
                    ...
                    $('#selectedBillCodeId').val(null);
                                ...
                                async addNewField() {
                                    ...
                                    this.addbuttonactive=false;
                                        ...
                                        this.virtualSelect(pint, 'cost_type', merchant_cost_types, null);
