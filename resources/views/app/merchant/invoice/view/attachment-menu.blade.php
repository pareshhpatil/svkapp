        ...
        <ul class="tree">
          ...
          @foreach ($docs as $row)
            ...
            <a onclick="myFunction('{{$row['id']}}','{{$row['id']}}')" class="popovers" data-placement="top" data-container="body" data-trigger="hover"  data-content="{{$row['full']}}">
              <label   id="l{{$row['id']}}" class=" tree_label @if(in_array($row['id'], $selectedDoc)) active1  @endif" for="{{$row['title']}}">{{$row['title']}}</label>
              <div id="arrow{{$row['id']}}" style="float: right;" class='@if(in_array($row['id'], $selectedDoc))fa fa-angle-down  active1 @else fa fa-angle-right @endif'></div>
            </a>
              @if(!empty($row['menu']))
              <ul id="ul{{$row['id']}}" style="display:@if(in_array($row['id'], $selectedDoc)) block @else none  @endif">
                ...
                <li >
                  ...
                  <label  for="{{$row2['title']}}" id="l{{$row2['id']}}"  class=" tree_label @if(in_array($row2['id'], $selectedDoc)) active1  @endif">{{$row2['title']}}</label>
                  <div id="arrow{{$row2['id']}}" style="float: right;" class='@if(in_array($row2['id'], $selectedDoc))fa fa-angle-down  active1 @else fa fa-angle-right  @endif'></div>
                  </a>
                  @else
                  <a  id="a{{$row2['id']}}" class=" @if(in_array($row2['id'], $selectedDoc)) aclass active1  @endif" href="@if($row2['link']!='')#tab_{{$row2['id']}}@else javascript:; @endif" data-toggle="tab">
                  ...
                  @endif
                  @if(!empty($row2['menu']))
                  <ul id="ul{{$row2['id']}}" style="display:@if(in_array($row2['id'], $selectedDoc)) block @else none  @endif">
                    ...
                    <li onclick="removeactive('{{$row['id']}}','{{$row2['id']}}','{{$row3['id']}}');" class="popovers"  data-placement="top" data-container="body" data-trigger="hover"  data-content="{{$row3['full']}}">
                      <a id="a{{$row3['id']}}" class=" @if(in_array($row3['id'], $selectedDoc)) aclass active1   @endif" href="@if($row3['link']!='')#tab_{{$row3['id']}}@else javascript:; @endif" data-toggle="tab">
