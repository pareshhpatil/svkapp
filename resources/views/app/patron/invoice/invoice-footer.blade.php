                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <input type="hidden" class="displayonly" id="c_fixed_amount"
                                                    value="{{$info['coupon_details']['fixed_amount']}}" />
                                            ...
                                            <td>
                                                <a class="btn blue pull-right" href="javascript:" id="btn_apply_coupun"
                                                    style="display: block;" onclick="handleCoupon(1,{{$info['coupon_details']['type']}});">Apply
                                                    Coupon <i class="fa fa-check"></i></a>
                                                <a class="btn red pull-right" href="javascript:" id="btn_remove_coupon"
                                                    style="display: none;" onclick="handleCoupon(2,{{$info['coupon_details']['type']}});">Remove
                        ...
                        @if(isset( $metadata['plugin']['has_coupon']) &&  isset($info['coupon_details']['coupon_id']))
                            <div class="row no-margin">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <button onclick="return validateCoupon('{{$info['merchant_user_id']}}', '{{$info['grand_total']}}');"
                    ...
                    <div class="col-md-12 invoice-block">
                        @if(isset($metadata['plugin']['has_digital_certificate_file']) && $metadata['plugin']['has_digital_certificate_file']==1)
                            <a class="btn btn-link hidden-print margin-bottom-5" target="_BLANK" style=""
                                href="/patron/invoice/download/{{$info['Url']}}/2 @if(isset($info['gtype'])) /{{$info['gtype']}}@endif">
                                Print
                            </a>
                            <a class="btn btn-link hidden-print margin-bottom-5" style=""
                                href="/patron/invoice/download/{{$info['Url']}} @if(isset($info['gtype'])) /1/{{$info['gtype']}}@endif">
                               Save as PDF
                            </a>
                       ...
                       @endif
                        ...
                        Pay now 
                            ...
                            @if( $info['payment_request_status']==6)
                                <a id="onlinepay" href="/patron/paymentrequest/view/{{$info['invoice_link']}}"
...
{{-- to do paresh sir --}}
            {{-- @if( $info['show_ad']==1)
                <div class="col-md-12 center">
                    <a target="_BLANK" href="/referrer/campaign/{{$info['payment_request_id']}}/1"><img src="/images/adv/ditto.jpg"
    ...
    <div class="col-md-12 no-margin no-padding" style="text-align: left;max-width: 909px;margin-top:10px;">
        @if ($info['paid_user']==0)
            <div onclick="location.href = '/merchant/register';" style="cursor: pointer;"
                ...
                <p>
                    <b>If you would like to collect online payments for your business, <a target="_BLANK" href="/merchant/register"><u>register now</u></a> on Swipez.</b>
                </p>
            </div>
        @endif
