...
.divide-red-400 > :not([hidden]) ~ :not([hidden]) {
    border-color: {{$info['design_color']}} !important;
}
    ...
    <table style="width: 100%"  role="presentation">
                <tr>
                  <td align="center" style="@if($viewtype=='mailer')padding-top: 30px; padding-bottom: 30px;@endif background-color: #F7F8F8"> 
                    <table class="sm-w-full" style="width: 700px" >
                <tr>
                  <td style="border: 1px solid gray; background-color: #fff; ">
                    <table width="100%" style="height: 20px; width: 100%; background-color: {{$info['design_color']}}">
                      
                    </table>
                     <table width="100%">  
                    <td style="margin-top: 16px; padding: 16px">
                        <table  role="presentation">
                            <tr>  
                                <td style="min-width: 200px;vertical-align: top !important; padding-right: 10px">
                                 <h1 style="margin-top: 0; font-size: 24px; font-weight: 600; color: {{$info['design_color']}}">
                                ...
                                <span style="margin-top: 8px; color: {{$info['design_color']}}">{{$item['column_name']}}</span> <br>
                                <span style="font-size: 14px; color: #000">{{$item['value']}}</span>
                                <br>
                              @endif
                                @endforeach
                                @foreach ($metadata['invoice'] as $item)
                             ...
                             @php $previous_due_col=$item['column_name']; @endphp
                                 @else
                                 <div style="margin-top: 5px; font-size: 14px; color: {{$info['design_color']}}">{{$item['column_name']}}</div>
                                ...
                                <span style="font-size: 14px; color: #000"> <span style="font-family:@if($info['currency_icon']=='₹')DejaVu Sans;@endif sans-serif;">{{$info['currency_icon']}}</span>{{number_format($item['value'],2)}}</span> <br>
                                @elseif($item['datatype']=='date')
                 
                                <span style="font-size: 14px; color: #000"> <x-localize :date="$item['value']" type="date" /></span> <br> 
                                @else
                             
                                <span style="font-size: 14px; color: #000"> {{$item['value']}} @if($item['datatype']=='percent') %@endif</span> <br>  
                                @endif
                               @endif
                                @endif
                                @endif
                                @endforeach
                                 ...
                                 <td style="vertical-align: top !important; border-left: 1px solid {{$info['design_color']}}; padding-left: 15px">
                                <h1 style="margin-top: 4px; font-size: 14px; text-transform: uppercase; color: {{$info['design_color']}}">Supplier</h1>
                ...
                @endforeach 
                                 ...
                                 <div style="font-weight: 600;margin-top:12px;margin-bottom:5px;font-size: 14px; text-transform: uppercase; color: {{$info['design_color']}}">Client</div>
                             
                                 <span style=" font-size: 14px; font-weight: 700">{{$metadata['customer'][1]['value'] }}</span> <br>
                            ...
                            <td style="min-width:118px;vertical-align: top !important; padding-left: 15px">
                                @if(isset($info['image_path']) && !empty($info['image_path']))
                                <section style="display: flex; flex-wrap: wrap; justify-content: center">
                            <img @if($viewtype=='mailer') width="190" @endif @if($viewtype!='mailer')src="data:image/png;base64,{{$info['logo']}}" @else src="{{env('APP_URL')}}/uploads/images/logos/{{$info['image_path']}}" @endif  style=" object-fit: scale-down;max-width:130px" alt="">
                                </section>
                                @endif
                            </td>                        </tr>
                        </table>                        
                            ...
                            <table  style="border-spacing: 0; margin-left: auto; margin-right: auto; width: 100%; overflow: hidden;  border-radius: 8px; background-color: #fff" cellpadding="0"  role="presentation">
                                    <thead>
                                        <tr >
                                            ...
                                            <th style="min-width:60px;border-bottom: 2px solid {{$info['design_color']}}; padding-left: 2px; padding-right: 2px; padding-top: 8px; padding-bottom: 8px;   text-align:center; font-size: 14px; font-weight: 600; color: {{$info['design_color']}}"> {{$item}} </th>
                                            @endforeach
                                        </tr>
                                    </thead>
                                    <tbody >
 ...
 @isset($info['narrative'])
            ...
            <div style="flex: 1 1 0%; font-size: 14px; color: rgb(39, 38, 38)"><b>Amount(in words) :</b> {{$info['absolute_cost_words']}}</div>
                            ...
                            @endif
                                ...
                                <table  width="100%" style="border: 2px dotted {{$info['design_color']}}; margin-top: 16px; "  role="presentation">
                        <tr>
                        <td>
                            <table width="100%">
                            <td style="flex: 1 1 0%; padding-left: 8px; padding-right: 8px; font-size: 14px; text-transform: uppercase; color: {{$info['design_color']}}">Due Date:</td>  
                            </table>                   
                        </td>
                       
                            <td style="text-align: right"> 
                                <table width="100%">
                        <td style="text-align: right;padding-top: 4px; padding-right: 10px;padding-bottom: 4px; font-size: 20px; font-weight: 500; color: {{$info['design_color']}}">{{$info['due_date']}}</td>  
                                </table>                         
                    </td>
                            <td style="text-align: center"> 
                                <table width="100%">
                        <td style="flex: 1 1 0%; padding-top: 8px; padding-bottom: 8px; font-size: 14px; text-transform: uppercase; color: {{$info['design_color']}}">Amount Due:</td>  
                                </table>                     
                    </td>
                        <td style="text-align: right">   
                            <table width="100%">   
   <td style="text-align: right;padding-left: 8px; padding-right: 10px; font-size: 20px; font-weight: 500; color: {{$info['design_color']}}">
    <strong style="font-family:@if($info['currency_icon']=='₹')DejaVu Sans;@endif sans-serif;">{{$info['currency_icon']}}</strong>{{number_format($info['absolute_cost'],2)}}
           ...
           <tr> 
            ...
            <table width="100%">
                  <td style="background-color: #e5e7eb; padding: 10px">
                    <table style="width: 100%"  role="presentation">
           ...
           <tr>
            ...
            <td style="border-left: 1px solid {{$info['design_color']}}; padding: 15px; vertical-align: top">
                <table  role="presentation">
                    @isset($info['payment_mode'])
                    <tr>
                        <td>
                <span style="margin-top: 0; font-size: 14px; color: {{$info['design_color']}}">PAYMENT METHOD</span>
                <br>
                     <span style="font-size: 14px; color: #000">{{$info['payment_mode']}}</span>        </td>
    </tr>
    @endisset
    @isset($info['transaction_id'])
    <tr>
        <td style="padding-top: 10px">
                <span style="margin-top: 0; font-size: 14px; color: {{$info['design_color']}}">ORDER NUMBER</span>
                <br>
                <span style="font-size: 14px; color: #000">{{$info['transaction_id']}}</span>        </td>
    </tr>
    @endisset
                </table>            </td>
                @endisset
