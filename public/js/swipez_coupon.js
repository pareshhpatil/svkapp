...
{
    document.createStyleSheet('https://h7sak8am43.swipez.in/tmp/cudu_swipez.css?v=4');
} else
{
    var styles = "@import url('https://h7sak8am43.swipez.in/tmp/cudu_swipez.css?v=4');";
...
function cudu_store()
{
    ...
    var swipezTKey = document.getElementById('swipez-offer').getAttribute("swipez-trans-key");
    ...
    //alert('https://h7sak-api.swipez.in/api/v1/coupon/store/'+swipezMerchantKey+'/'+swipezTKey);
    xhr.open('GET', 'https://h7sak-api.swipez.in/api/v1/coupon/store/' + swipezMerchantKey + '/' + swipezTKey, true)
    xhr.onload = function ()
    {
        if (this.status === 200)
        {
            ...
            $.each(stores['stores'], function (index, value) {
                ...
                offer_str_li += '<tr><td style="text-align:center;"><span><button class="button" onclick="getStoreOffers(' + store_id + ',' + swipezMKey_str + ');">Get Offers</button></span></td></tr></table></li>';
            });
            offer_str_li += '</ul>';
...
function cudu_exclusive_offers()
{
    ...
    let xhr = new XMLHttpRequest;
    xhr.open('GET', 'https://h7sak-api.swipez.in/api/v1/coupon/exclusiveoffer/' + swipezMerchantKey + '/' + swipezTKey, true)
    xhr.onload = function ()
    {
        if (this.status === 200)
        {
            ...
            for (item in offers)
            {
                ...
                offer_str_li += '<tr><td style="text-align:center;"><span><button class="button" onclick="getCouponCode(' + offer_public_id + ',' + swipezMKey_str + ',' + img_name + ',' + title + ',' + description + ',' + outlink_url + ');">Get Deal</button></span></td></tr></table></li>';
...
function cudu_offers()
{
    ...
    //alert('https://h7sak-api.swipez.in/api/v1/coupon/offer/'+swipezMerchantKey+'/'+swipezTKey); return false;
    let xhr = new XMLHttpRequest;
    xhr.open('GET', 'https://h7sak-api.swipez.in/api/v1/coupon/offer/' + swipezMerchantKey + '/' + swipezTKey, true)
    xhr.onload = function ()
    {
        if (this.status === 200)
        {	//console.log(JSON.parse(this.responseText));
...
function getCouponCode(id, Mkey, img_name, title, description, outlink_url)
{
    window.scrollTo(0, 0);
    var o_p_id = id;
    var m_key = Mkey;
    let xhr = new XMLHttpRequest;
    //alert('https://h7sak-api.swipez.in/api/v1/coupon/getcode/'+o_p_id+'/'+m_key); return false;
    xhr.open('GET', 'https://h7sak-api.swipez.in/api/v1/coupon/getcode/' + o_p_id + '/' + m_key, true)
    xhr.onload = function ()
    {
        if (this.status === 200)
        {
...
function getStoreOffers(store_id, Mkey)
{
    ...
    //alert('https://h7sak-api.swipez.in/api/v1/coupon/getstoreoffer/'+store_id+'/'+m_key);
    xhr.open('GET', 'https://h7sak-api.swipez.in/api/v1/coupon/getstoreoffer/' + store_id + '/' + m_key, true)
    xhr.onload = function ()
    {
        if (this.status === 200)
        {
