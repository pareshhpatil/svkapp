        ...
        $.each(csi_codes, function (value, arr) {
            ...
            try {
                if (arr.code != '') {
                    ...
                    produ_text = produ_text + '<option ' + selected + ' value="' + arr.id + '">' + arr.code + ' | ' + arr.title + '</option>';
                }
            } catch (o) {
            }
        });
...
function addRowinCalcTable(ind) {
    ...
    $('input[name="pint[]"]').each(function (indx, arr) {
        ...
        if (ind != int) {
            ...
            row = row + '<td class="td-c"><input type="hidden" name="calc-pint[]" value="' + int + '" id="calc-pint' + int + '"><input type="checkbox" name="calc-checkbox[]" value="' + int + '" id="calc' + int + '" onclick="inputCalcClicked(' + int + ',' + getamt(document.getElementById('original_contract_amount' + int).value) + ')"></td><td class="td-c">' + bill_code + '</td><td class="td-c">' + document.getElementById('description-hidden' + int).value + '</td><td class="td-c">$' + document.getElementById('original_contract_amount' + int).value + '</td>'
        }
        newDiv.innerHTML = row;
        mainDiv.appendChild(newDiv);
...
function setAdvanceDropdownContract(numrow) {
    try {
        $('.productselect').select2({
            tags: true,
