...
date_format(end_time,'%H:%i') as drop_time,GROUP_CONCAT(ps.employee_name ORDER BY p.id SEPARATOR ', ') AS passengers,COUNT(p.id) AS employee_count,z.company_slab,escort,r.id as logsheet_number,company_amount,r.note from ride r inner join vehicle v on v.vehicle_id=r.vehicle_id 
inner join ride_passenger p on r.id=p.ride_id inner join passenger ps on p.passenger_id=ps.id and ps.passenger_type=1 and p.status<>3
