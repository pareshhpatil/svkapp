...
class RideModel extends ParentModel
{
    public function getRidePassenger($ride_id)
    {
        $retObj = DB::table('ride_passenger as p')
            ...
            ->select(DB::raw('p.id,pr.address,pr.mobile ,p.status,p.otp,p.pickup_time as pickup_date_time,TIME_FORMAT(p.pickup_time, "%h %i %p") as pickup_time ,rating,p.cab_time as cab_date_time,TIME_FORMAT(p.cab_time, "%h %i %p") as cab_time,p.in_time as in_date_time ,TIME_FORMAT(p.in_time, "%h %i %p") as in_time,p.drop_time as drop_date_time ,TIME_FORMAT(p.drop_time, "%h %i %p") as drop_time ,
