...
class Vehicle extends Model
{
    ...
    public function getFuelList($from_date, $to_date, $admin_id, $vehicle_id = 0)
    {
        ...
        if ($vehicle_id > 0) {
            $retObj = $retObj->where('v.vehicle_id', $vehicle_id);
        }
        //->where('date', ">=", $from_date)
        //->where('date', "<=", $to_date)
        $retObj = $retObj->select(DB::raw('fuel_id,v.number,e.name,a.date,litre,rate,amount,s.name as paymentsource'))
            ->get();
        return $retObj;
    }
}
