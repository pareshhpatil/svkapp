...
class SubcontractRequest extends ParentModel
{
    ...
    public function getSubContractDetail($id)
    {
        $retObj = DB::table('sub_contract as c')
            ->select(DB::raw('c.*,p.project_name as project_name,vendor_name,vendor_code'))
            ->join('project as p', 'p.id', '=', 'c.project_id')
            ->join('vendor as v', 'v.vendor_id', '=', 'c.vendor_id')
            ->where('c.sub_contract_id', $id)
            ->first();
        return $retObj;
    }
    ...
    public function getPreviousSubContractRequestPayment($merchant_id, $contract_id, $request_id)
    {
        $retObj = DB::table('subcontract_payment_request')
            ->select(DB::raw('request_id'))
            ->where('merchant_id', $merchant_id)
            ->where('sub_contract_id', $contract_id)
            ->where('request_id', '<>', $request_id)
            ->whereNotIn('payment_request_status', [11, 3])
            ->orderBy('request_id', 'desc')
            ->first();
        if (!empty($retObj)) {
            return $retObj->request_id;
        } else {
            return false;
        }
    }
    ...
    public function getPreviousSubcontractRequest($merchant_id, $sub_contract_id, $request_id)
    {
        $currentInvoice = DB::table('subcontract_payment_request')
            ->where('merchant_id', $merchant_id)
        ...
        if (!empty($currentInvoice)) {
            ...
            $retObj =  $retObj->orderBy('request_id', 'desc')
                ->first();
        }
    ...
    public function saveSubcontractRequest($merchant_id,  $data, $user_id)
    {
        $date = date_create($data['bill_date']);
        $bill_date = date_format($date, "Y-m-d");
        $date = date_create($data['due_date']);
        $due_date = date_format($date, "Y-m-d");
        $id = DB::table('subcontract_payment_request')->insertGetId(
            [
    ...
    public function updateSubcontractRequest($id,  $data, $user_id)
    {
        $date = date_create($data['bill_date']);
        $bill_date = date_format($date, "Y-m-d");
        $date = date_create($data['due_date']);
        $due_date = date_format($date, "Y-m-d");
        DB::table('subcontract_payment_request')->where('request_id', $id)
            ->update([
                'request_number' => $data['request_number'],
                'bill_date' => $bill_date,
                'due_date' => $due_date,
    ...
    public function updateSubcontractRequestDetail($request_id, $amount, $ids, $previous_amount = 0)
    {
        $amount = (is_numeric($amount)) ? $amount : 0;
        DB::table('subcontract_payment_request')->where('request_id', $request_id)
            ->update([
                'absolute_cost' => $amount - $previous_amount,
                'basic_amount' => $amount,
                'grand_total' => $amount,
    ...
    public function saveConstructionParticular($data, $request_id, $user_id, $type = 'save')
    {
        ...
        if ($type == 'draft') {
            ...
            unset($array['payment_request_id']);
            unset($array['billed_transaction_ids']);
            $array['subcontract_request_id'] = $request_id;
            $array['task_number'] = $data['task_number'];
            $table = 'subcontract_request_payment_particular';
        }
