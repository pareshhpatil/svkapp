...
class Order extends ParentModel
{
    ...
    public function saveNewOrder($data, $merchant_id, $user_id, $type = null)
    {
        $table = 'order';
        if ($type == 'subcontract') {
    ...
    public function updateOrder($data, $merchant_id, $user_id, $id, $type = null)
    {
        $table = 'order';
        if ($type == 'subcontract') {
            $table = 'subcontract_change_order';
    ...
    public function getPrivilegesSubcontractOrderList($merchant_id, $from_date, $to_data, $contract, $privilegesIDs = [])
    {
        $table = 'subcontract_change_order';
        $userRole = Session::get('user_role');
        $project_cond = "";
        ...
        if ($userRole != 'Admin' && !in_array('all', $privilegesIDs)) {
            $retObj =  DB::select("SELECT a.*,vendor_name as company_name, a.invoice_status, p.project_name, p.project_id  project_code,sub_contract_code as contract_code, a.order_no, vendor_name as name , c.vendor_id as customer_id, c.vendor_code as customer_code
        FROM `" . $table . "` a
        join sub_contract b on a.contract_id  = b.sub_contract_id
        join project p on b.project_id  = p.id
    ...
    public function changeOrderApproveStatus($order_id, $date, $status, $unapprove_message, $type = '')
    {
        $table = 'order';
        if ($type == 'subcontract') {
            $table = 'subcontract_change_order';
