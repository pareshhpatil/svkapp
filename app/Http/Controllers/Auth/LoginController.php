...
class LoginController extends Controller
{
    ...
    */
    ...
    public function userSwitch($link)
    {
        $model =  new ApiModel();
        $user_id = Encryption::decode($link);
        if ($user_id > 0) {
    ...
    public function driverLogin($link, $driver_id)
    {
        $model =  new ApiModel();
        $date = Encryption::decode($link);
        if ($date != date('Y-m-d')) {
            return redirect('/404');
        }
        $user_id = $model->getColumnValue('users', 'parent_id', $driver_id, 'id', ['user_type' => 4]);
        if ($user_id > 0) {
            Auth::loginUsingId($user_id, true);
            $user = Auth::user();
            Session::put('user_id', $user->id);
        ...
        if ($data != false) {
            ...
            if (in_array($request->mobile, $test_array)) {
                ...
                if (strlen($mobile) == 10) {
                    ...
                    $apicontroller->sendWhatsappMessage($mobile, 'mobile', 'otp_svk', $params, (string) $otp, 'en_US');
                }
            }
