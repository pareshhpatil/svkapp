...

    {
...

		{
...

			{
				...
				$this->master_model->savePaymentStatement($transaction->source_id, $transaction->transaction_id, $date, $transaction->amount, 'Debit', 'Expense', "Paid to :" . $employee->name . ' for ' . $transaction->narrative, $transaction->created_by);
...

		{
			...
			$bill_model->updateTransaction(2, $request_id, $payment_transaction->amount, 'NEFT', $transaction->source_id, $request->eventTime, 100);
			...
			$this->master_model->savePaymentStatement($transaction->source_id, $transaction->transaction_id, $date, $transaction->amount, 'Credit', 'Reverse', "Reverse payment from :" . $employee->name, $transaction->created_by);
