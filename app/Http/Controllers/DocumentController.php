...
class DocumentController extends Controller
{
    ...
    public function createMulti()
    {
        ...
        $data['driver_list'] = $this->model->getTableList('driver', 'is_active', 1, 0, []);
        $data['vehicle_list'] = $this->model->getTableList('vehicle', 'is_active', 1, 0, []);
        return view('web.document.create_multi', $data);
    }
    ...
    public function saveMulti(Request $request)
    {
        ...
        $projectId = $request->project_id !== null && $request->project_id !== '' ? (int) $request->project_id : null;
        $assignedUserId = $request->assigned_user_id !== null && $request->assigned_user_id !== '' ? (int) $request->assigned_user_id : null;
        ...
        foreach ($items as $idx => $row) {
            ...
            $subtype = isset($row['document_type']) ? (string) $row['document_type'] : '';
            if ($name === '' || $subtype === '' || !array_key_exists($subtype, $allowedSubtypes)) {
                continue;
            }
            ...
            $array = [
                ...
                'issue_date' => !empty($row['issue_date']) ? $this->sqlDate($row['issue_date']) : null,
                'expiry_date' => !empty($row['expiry_date']) ? $this->sqlDate($row['expiry_date']) : null,
        ...
        if ($saved === 0) {
            return redirect()->back()->withInput()->withErrors(['items' => 'Add at least one document row with name and document type.']);
        }

        return redirect('/document/list')->withSuccess($saved . ' document(s) saved successfully');
    }
