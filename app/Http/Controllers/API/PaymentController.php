...
class PaymentController extends APIController
{
    ...
    public function status(Request $request)
    {
        ...
        if (!isset($request->APIdata['id'])) {
            ...
            return response()->json($this->APIResponse('ER01008'));
        }
        ...
        if (isset($valid_mode[$request->APIdata['transaction_type']])) {
            ...
            return response()->json($this->APIResponse('ER02020'));
        }

        try {
            $result = $paymentModel->getPaymentStatus($request->merchant_id, $request->user_id, $id, $mode);
            if (empty($result)) {
                return response()->json($this->APIResponse('ER02021'));
            }
            return response()->json($this->APIResponse('',$result));
        } catch (Exception $e) {
