                    ...
                    <form class="form-inline" role="form" action="" method="post">
                        <div class="form-group">
                            <input class="form-control form-control-inline input-sm rpt-date" id="daterange"
                                autocomplete="off" data-date-format="dd M yyyy" name="date_range" type="text"
                        ...
                        <table class="table table-striped  table-hover" id="table-small">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        {$customer_default_column.email|default:'Email ID'}
                                    </th>

                                    <th>
                                        {$customer_default_column.mobile|default:'Mobile No'}
                                    </th>
                                    <th>
                                        Amount
                                    </th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {if $v.status=="Paid online"  || $v.status=="Refund" || $v.status=="Charge back" || $v.status=="Paid offline"}
                                                <a href="/merchant/transaction/receipt/{$v.transaction_id}/website"
                                                    title="Receipt" class="iframe">{$v.customer_name}
                                                </a>
                                            {else}
                                                {$v.customer_name}
                                            {/if}
                                        ...
                                        <td class="td-c">
                                            ...
                                            {else if {$v.status}=="Paid offline"}
                                                ...
                                                <a id="{$v.pay_transaction_id}" onclick="updateRespond(this.id);"><span
                                        ...
                                        <td>
                                            ...
                                            {if $v.status=="Paid online"  || $v.status=="Refund" || $v.status=="Charge back" || $v.status=="Paid offline"}
                                                {if $v.payment_request_id!=''}
                                                    <a href="/merchant/transaction/invoice/{$v.payment_request_id}" title="Invoice"
                                                        class="btn btn-xs blue iframe"><i class="fa fa-table"></i> </a>
                                                {else}
                                                    {if $plan_invoice_create==1 && $v.status=="Paid online"}
                                                        <a href="/merchant/transaction/generateinvoice/{$v.transaction_id}/plan"
                                                            title="Create Invoice" class="btn btn-xs green"><i class="fa fa-plus"></i>
                                                        </a>
                                                    {/if}
                                                {/if}
                                                <a href="/merchant/transaction/receipt/{$v.transaction_id}/website"
                                                    title="Receipt" class="btn btn-xs green iframe"><i class="fa fa-file-o"></i>
                                                </a>
                                                <a onclick="document.getElementById('sendanchor').href = '/merchant/transaction/sendreceipt/plan/{$v.transaction_id}'"
                                                    href="#basic" data-toggle="modal" title="Send Receipt"
                                                    class="btn btn-xs blue"><i class="fa fa-send"></i></a>
                                                {if $v.status=="Paid online" && $pg_vendor_id==0}
                                                    <a href="#refund" title="Refund" class="btn btn-xs yellow"
                                                        ...
                                                        data-toggle="modal"><i class="fa fa-undo"></i></a>
                                                {/if}
                                            {/if}

                                            {if $v.status=="Failed"}
                                                <a href="/merchant/transaction/reconcile/{$v.transaction_id}"
                                                    class="btn btn-xs yellow" title="Reconcile"><i
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <a href="" onclick="document.getElementById('close').click();
                                            document.getElementById('loader').style.display = 'block';" id="sendanchor"
                    class="btn blue">Confirm</a>
            </div>
        </div>
        <!-- /.modal-content -->
    </div>
