{if $info.template_type!='scan'}
    ...
    <div class="table-scrollable">
        <table class="table table-bordered table-hover">
            ...
            <tbody id="new_tax">
                {$int =1}
                {foreach from=$default_tax item=v}
                    <tr>
                <input type="hidden" name="tax_detail_id[]" value="0"/>
                <input type="hidden" name="tax_type[]" value="{$tax_list.{$v}.tax_type}"/>
                <td>
                    <div class="input-icon right">
                        <select style="width:100%;" onchange="calculatetax();"  name="tax_id[]" data-placeholder="Select..." class="form-control  taxselect" >
                            <option value="">Select Tax</option>
                            {foreach from=$tax_list key=k item=vt}
                                <option {if $v==$k} selected {/if} value="{$k}">{$vt.tax_name}</option>
                            {/foreach}
                        </select>
                    </div>
                </td>
                <td>
                    <div class="input-icon right">
                        <input type="number" step="0.01" max="100" name="tax_percent[]" placeholder="Add tax %" readonly=""
                               value="{$tax_list.{$v}.percentage}"  class="form-control ">
                    </div>
                </td>
                <td>
                    <input type="text" {$validate.money} name="tax_applicable[]" placeholder="Add applicable on" onblur="calculatetax();" class="form-control ">
                </td>
                <td>
                    <input type="text" name="tax_amt[]" readonly class="form-control " >
                </td>
                <td class="td-c">
            ...
            <tr class="warning">
                <td><div class="input-icon right">
                        <input type="text" readonly  value="{$info.tax_total}" class="form-control " placeholder="Enter total label">
                    </div>
                </td>
                <td></td>
                <td></td>
                <td>
                    ...
                    <input type="text" id="totaltaxcost" name="totaltax" value="{$post.totaltax}" class="form-control " readonly>
                </td>
                <td></td>
            </tr>
        </table>
...
<div class="display-none" id="tax_div">
    <select style="width: 100%;" onchange="calculatetax();" name="tax_id[]" data-placeholder="Select..." class="form-control  taxselectdefault" >
                    ...
                    <tbody id="new_debit">
                        ...
                        {foreach from=$debit item=v}
                            <tr>
                                ...
                                <td>
                                    <input type="text" {$validate.money} name="deduct_applicable[]" placeholder="Add applicable on" value="" id="applicabledebitamount{$int}" onblur="calculatedebit({$int})" class="form-control ">
                                </td>
                                <td>
                                    <input type="text" type="number" step="0.01" max="100" name="deduct_total[]" readonly="" value="" id="totaldebit{$int}" class="form-control " >
                                </td>
                    ...
                    {section name=sec1 loop=$plugin.supplier}
                        <tr id="row{$supplierlist[sec1].supplier_id}">
                            ...
                            <td class="td-c"><a href="javascript:;" id="{$supplierlist[sec1].supplier_id}" onclick="removesupplier(this.id);
                                    $(this).closest('tr').remove();" class="btn btn-sm red"> <i class="fa fa-times"> </i> Delete row </a> </td> 
                        </tr> 
                    {/section}
                </tbody>
            </table>
        ...
        <div class="row mb-2">
            <div class="col-md-12">
                <div class="form-group form-horizontal">
                    ...
                    <div class="col-md-6">
                        <textarea class="form-control" type="text" maxlength="200" name="custom_sms" placeholder="Payment request SMS">{if $info.custom_sms!=''}{$info.custom_sms}{else}You have received a payment request from %COMPANY_NAME% for amount %TOTAL_AMOUNT%. To make an online payment, access your bill via %SHORT_URL%{/if}</textarea>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            {if !empty($plugin.reminders)}
                                {foreach from=$plugin.reminders key=k item=v}
                                    <tr>
                                        <td>
                                            <div class="input-icon right">
                                                <input type="number" step="1" min="0" max="50" name="reminders[]" id="rm{$k}" value="{$k}" class="form-control"   >
                                            </div>
                                        </td>
                                        <td>
                                            <div class="input-icon right">
        ...
        <div class="row">
            <div class="col-md-2 w-auto">
                <div class="form-group">
                    ...
                    <input type="text" id="totalamount" name="grand_total" value="{$post.amount}" readonly class="form-control">
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
                    <p>Grand total</p>
                    <input type="text" id="grandtotal" value="{$post.grand_total}" readonly class="form-control">
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
                    <p>Narrative</p>
                    <div class="input-icon right">
                        <input type="text" name="invoice_narrative" value="{$post.narrative}" class="form-control" placeholder="Enter narrative ">
                    </div>
                </div>
            </div>
            <div class="col-md-2 w-auto">
                <div class="form-group">
                    <p>Notify customer</p>
                    <div class="input-icon">
                        <input type="checkbox" id="notify_" onchange="notifyPatron('notify_');" value="1" checked class="make-switch" data-size="small">  
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-3 pull-right">
                <div class="pull-right">
                    ...
                    <input type="hidden" id="totalcostamt" name="totalcost" value="0">
                    <input type="hidden" id="is_notify_" name="notify_patron" value="1"/> 
                    <input type="hidden" name="request_type" value="{if $request_type==2}2{else}1{/if}"/> 
                    <input type="submit"  class="btn blue" id="subbtn" value="Save & Send">
