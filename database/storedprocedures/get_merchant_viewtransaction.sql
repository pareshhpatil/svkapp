CREATE DEFINER=`root`@`localhost` PROCEDURE `get_merchant_viewtransaction`(_userid varchar(10),_from_date date , _to_date date,_user_name varchar(10) ,_status int,_payment_request_type INT,_franchise_id INT,_bulk_id INT)
BEGIN
...
if(_status <> -1)then
	if(_status=2) then
		...
		absolute_cost,deduct_amount,is_availed,payment_transaction_status,narrative,date,last_update_on,mode,quantity,offline_response_type) 
		select offline_response_id,payment_request_id,estimate_request_id,merchant_user_id,customer_id,amount,deduct_amount,is_availed,
		2,narrative,settlement_date,last_update_date,'offline',quantity,offline_response_type from offline_response where  is_active=1 and merchant_id=_userid and offline_response_type<>4 and DATE_FORMAT(settlement_date,'%Y-%m-%d') >= DATE_FORMAT(_from_date,'%Y-%m-%d') and DATE_FORMAT(settlement_date,'%Y-%m-%d')<= DATE_FORMAT(_to_date,'%Y-%m-%d') ;
        end if;
	else
		...
		payment_transaction_status,narrative,created_date,last_update_date,'online',pg_id,payment_mode from payment_transaction where payment_transaction_status<>0 and merchant_id=_userid  and  payment_transaction_status=_status and DATE_FORMAT(created_date,'%Y-%m-%d') >= DATE_FORMAT(_from_date,'%Y-%m-%d') and DATE_FORMAT(created_date,'%Y-%m-%d') <= DATE_FORMAT(_to_date,'%Y-%m-%d') ;
    end if;
	if(_status=3) then
		insert into merchant_view_transaction(pay_transaction_id,payment_request_id,estimate_request_id,user_id,customer_id,
	...
	absolute_cost,deduct_amount,unit_price,is_availed,quantity,payment_transaction_status,narrative,date,last_update_on,mode,pg_id,payment_mode) 
	select pay_transaction_id,payment_request_id,estimate_request_id,merchant_user_id,customer_id,patron_user_id,amount,deduct_amount,unit_price,is_availed,quantity,
	payment_transaction_status,narrative,created_date,last_update_date,'online',pg_id,payment_mode from payment_transaction where payment_transaction_status<>0 and merchant_id=_userid  and DATE_FORMAT(created_date,'%Y-%m-%d') >= DATE_FORMAT(_from_date,'%Y-%m-%d') and DATE_FORMAT(created_date,'%Y-%m-%d')<= DATE_FORMAT(_to_date,'%Y-%m-%d') ;

	insert into merchant_view_transaction(pay_transaction_id,payment_request_id,estimate_request_id,user_id,customer_id,
	absolute_cost,deduct_amount,is_availed,payment_transaction_status,is_reject,narrative,date,last_update_on,mode,quantity,offline_response_type) 
	select offline_response_id,payment_request_id,estimate_request_id,merchant_user_id,customer_id,amount,deduct_amount,is_availed,
	2,offline_response_type,narrative,settlement_date,last_update_date,'offline',quantity,offline_response_type from offline_response where is_active=1 and merchant_id=_userid  and DATE_FORMAT(settlement_date,'%Y-%m-%d') >= DATE_FORMAT(_from_date,'%Y-%m-%d') and DATE_FORMAT(settlement_date,'%Y-%m-%d')<= DATE_FORMAT(_to_date,'%Y-%m-%d') ;
